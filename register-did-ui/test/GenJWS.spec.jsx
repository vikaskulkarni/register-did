...
test("renders GenJWS as expected", () => {
  ...
  const tree = renderer
    .create(<GenJWS jwsValue="dummy data" resetNextButton={resetHandler} />)
    .toJSON();
...
test("when no jwt value, does not allow copy and rest", () => {
  ...
  expect(getByTestId("copy-icon")).toHaveAttribute(
    "class",
    "copy-icon-disabled"
  );
});
...
test("clicking on copy icon calls reset handler once", () => {
  ...
  const { getByTestId } = render(
    <GenJWS jwsValue="dummy data" resetNextButton={resetHandler} />
  );
